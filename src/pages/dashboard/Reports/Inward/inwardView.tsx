...
const TeamInwardDataReportsView = ({
  selectedDates,
  selectedTeamMembers,
  selectedStageStatus,
  selectedSeries,
  ...
  const filters = getFilter("inward_report");
  ...
  const handleApplyFilters = (data: any) => {
    ...
    setFilters("inward_report", updatedFilters);
  ...
  const canShareInward = useCheckUserPermission(
    PAGE_ID.DISPATCH,
    PERMISSION_TYPE.SHARE,
  );
  ...
  //     const rowData: any = {
  //       inward_number: item.cart_number || "XXXXXXX",
  //       customer_name:
  ...
  const fetchAccountOutstandingForExport = async (
    ...
    return fetchInwardReportForExport(
      filters.selectedDateArray,
      filters.checkedOptionsUser,
      filters.checkedOptionsStageStatus,
      MobileToken,
  ...
  const exportExcel = async () => {
    try {
      ...
      const exportData = await exportAllInwardData<IFlatCartItem>(
      ...
      const excelRows = (
        ...
        visibleColumns.forEach((col) => {
          ...
          const label =
            col.key === "cart_number" ? "Goods Received Note Number" : col.label;
        ...
        row[`Grand Total (${currencyName})`] = item.grand_total_wo_c || "-";
    ...
    defs.push(
      {
        ...
        body: (rowData: any) => (
          <span
            ...
            onClick={() => {
              if (!MobileFlag) {
                handelChangeShowModelInward(rowData);
              }
            }}
            title={`View ${title}`}
          >
  ...
  const {
    ...
    reorderColumns,
    resetColumns,
  } = useColumnPreferences("inward_report", baseColumnDefs);
  ...
  const handleMultiPrint = async () => {
    ...
    try {
      ...
      if (pdfmeOn) {
        ...
        await generateAndPrintInwardPdf(cartIdOrIds);
        return;
      }
  ...
  const printWithTemplate = async (templateId: number) => {
    ...
    setIsMultiPrintLoading(true);
    try {
      await generateAndPrintInwardPdf(cartIdOrIds, templateId);
    } finally {
      setIsMultiPrintLoading(false);
    }
  };
...
export default TeamInwardDataReportsView;
