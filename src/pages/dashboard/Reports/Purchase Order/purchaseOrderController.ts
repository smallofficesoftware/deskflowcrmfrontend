...
// Purchase Order (cart type 5) is pdfme-supported - same shape as
...
export const isPdfmeEnabledForPurchaseOrder = async (): Promise<boolean> => {
  if (!isPdfmeSupportedCartType(5)) return false;
  ...
  try {
    const { data } = await axiosInstance.post("get-feature-flag", {
      ...
      feature_key: "purchaseOrder_document_designer",
    });
...
export const fetchPurchaseOrderPdfmeTemplates = () =>
  fetchPdfmeTemplatesForPicker(5);
...
export const generateAndPrintPurchaseOrderPdf = async (
...
export const fetchCartReport = async (
  setCartData: TReactSetState<IFlatCartItem[]>,
  selectedDates: DateObject[] | any | undefined,
  selectedTeamMembers: string[] | null,
  selectedStageStatus?: string[] | null | undefined,
...
export const fetchReturnOrderReportForExport = async (
  selectedDates: any,
  selectedTeamMembers: string[] | null,
...
export const exportAllPurchaseOrderData = async <T>(
