...
import {
  fetchCartReport,
  fetchPurchaseOrderPdfmeTemplates,
  generateAndPrintPurchaseOrderPdf,
  IFlatCartItem,
  isPdfmeEnabledForPurchaseOrder,
...
const TeamPurchaseOrderDataReportsView = ({
  ...
  const handleMultiPrint = async () => {
    ...
    try {
      const pdfmeOn = await isPdfmeEnabledForPurchaseOrder();
      if (pdfmeOn) {
        const choices = await fetchPurchaseOrderPdfmeTemplates();
        if (choices.length > 1) {
          setPrintTemplateChoices(choices);
          setPendingPrintCartId(cartIdOrIds);
          return;
        }
        await generateAndPrintPurchaseOrderPdf(cartIdOrIds);
        return;
      }
  ...
  const printWithTemplate = async (templateId: number) => {
    ...
    setIsMultiPrintLoading(true);
    try {
      await generateAndPrintPurchaseOrderPdf(cartIdOrIds, templateId);
    } finally {
      setIsMultiPrintLoading(false);
    }
  };
            ...
            >
              ...
              <div className="d-flex gap-2 align-items-center">
                ...
                <div ref={dropdownRef} style={{ position: "relative" }}>
                  ...
                  >
                    <ExportExcelMenuItem
                      reportType="purchase_order_report"
                      filters={{
