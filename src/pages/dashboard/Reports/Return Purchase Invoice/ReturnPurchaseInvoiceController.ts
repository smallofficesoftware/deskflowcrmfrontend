...
// Return Purchase Invoice (cart type 7) is pdfme-supported - same shape as
// ListOrderView.tsx's openPrint. Only wired for a single id, same as
// ListOrderView.tsx - multi-select print keeps the legacy
// openPrint(ids.join(","), viewFormate) path above.
export const isPdfmeEnabledForReturnPurchaseInvoice = async (): Promise<boolean> => {
  if (!isPdfmeSupportedCartType(7)) return false;
  ...
  try {
    const { data } = await axiosInstance.post("get-feature-flag", {
      ...
      feature_key: "returnPurchaseInvoice_document_designer",
    });
...
export const fetchReturnPurchaseInvoicePdfmeTemplates = () =>
  fetchPdfmeTemplatesForPicker(7);
...
export const generateAndPrintReturnPurchaseInvoicePdf = async (
...
export const fetchReturnPurchaseInvoiceReportForExport = async (
  selectedDates: any,
  selectedTeamMembers: string[] | null,
  selectedStageStatus?: string[] | null,
  MobileToken?: string,
  ...
  if (selectedTrasactionModeOptions)
    payload.selectedTrasactionModeOptions = selectedTrasactionModeOptions;
...
export const exportAllReturnPurchaseInvoiceData = async <T>(
