...
import {
  exportAllReturnPurchaseInvoiceData,
  fetchCartReport,
  fetchReturnPurchaseInvoicePdfmeTemplates,
  fetchReturnPurchaseInvoiceReportForExport,
  generateAndPrintReturnPurchaseInvoicePdf,
  IFlatCartItem,
  isPdfmeEnabledForReturnPurchaseInvoice,
  openPrint,
} from "./ReturnPurchaseInvoiceController";
...
const TeamReturnPurchaseDataReportsView = ({
  selectedDates,
  selectedTeamMembers,
  selectedStageStatus,
  selectedSeries,
  ...
  const filters = getFilter("return_purchase_invoice");
  ...
  const handleApplyFilters = (data: any) => {
    ...
    setFilters("return_purchase_invoice", updatedFilters);
  ...
  useEffect(() => {
    if (!filters.startSearchDate || !filters.endSearchDate) {
      ...
      setFilters("return_purchase_invoice", {
        ...filters,
  ...
  const canDelReturnPurchaseInv = useCheckUserPermission(
    PAGE_ID.RETURN_PURCHASE_INVOICE,
  ...
  const fetchAccountOutstandingForExport = async (
    ...
    return fetchReturnPurchaseInvoiceReportForExport(
      filters.selectedDateArray,
      filters.checkedOptionsUser,
      filters.checkedOptionsStageStatus,
      MobileToken,
  ...
  const exportExcel = async () => {
    try {
      ...
      const exportData =
        await exportAllReturnPurchaseInvoiceData<IFlatCartItem>(
      ...
      const totalRow: any = {
        ...
        "Return Purchase INV Number": "Total",
        "Approval Status": "",
        "Company Name": "",
        "Customer Name": "",
        "Customer Phone": "",
      ...
      xlsx.utils.book_append_sheet(wb, ws, "Return Purchase Invoice Report");
      ...
      saveAs(
        ...
        `return_purchase_invoice_report_${Date.now()}.xlsx`,
      );
  ...
  const handleMultiPrint = async () => {
    ...
    try {
      const pdfmeOn = await isPdfmeEnabledForReturnPurchaseInvoice();
      if (pdfmeOn) {
        const choices = await fetchReturnPurchaseInvoicePdfmeTemplates();
        if (choices.length > 1) {
          setPrintTemplateChoices(choices);
          setPendingPrintCartId(cartIdOrIds);
          return;
        }
        await generateAndPrintReturnPurchaseInvoicePdf(cartIdOrIds);
        return;
      }
  ...
  const printWithTemplate = async (templateId: number) => {
    ...
    setIsMultiPrintLoading(true);
    try {
      await generateAndPrintReturnPurchaseInvoicePdf(cartIdOrIds, templateId);
    } finally {
      setIsMultiPrintLoading(false);
    }
  };
        ...
        <div>
          ...
          {isModalFilterVisible && (
            <CheckBoxFilterModal
              ...
              pageId={1}
              stageandStatusOrderType={10}
              filtershowSeriesOrderType={"return_purchase_invoice_prefix"}
              initialFilterData={{
                ...filters.filterData,
...
export default TeamReturnPurchaseDataReportsView;
