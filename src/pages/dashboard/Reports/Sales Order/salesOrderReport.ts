...
// Sales Order (cart type 2) is pdfme-supported - same shape as
...
export const isPdfmeEnabledForSalesOrder = async (): Promise<boolean> => {
  if (!isPdfmeSupportedCartType(2)) return false;
  ...
  try {
    const { data } = await axiosInstance.post("get-feature-flag", {
      ...
      feature_key: "salesOrder_document_designer",
    });
...
export const fetchSalesOrderPdfmeTemplates = () =>
  fetchPdfmeTemplatesForPicker(2);
...
export const generateAndPrintSalesOrderPdf = async (
