...
// Dispatch (cart type 9) is pdfme-supported - same shape as
...
export const isPdfmeEnabledForDispatch = async (): Promise<boolean> => {
  if (!isPdfmeSupportedCartType(9)) return false;
  ...
  try {
    const { data } = await axiosInstance.post("get-feature-flag", {
      ...
      feature_key: "dispatch_document_designer",
    });
...
export const fetchDispatchPdfmeTemplates = () =>
  fetchPdfmeTemplatesForPicker(9);
...
export const generateAndPrintDispatchPdf = async (
...
export const fetchDispatchReportForExport = async (
  selectedDates: any,
  selectedTeamMembers: string[] | null,
  selectedStageStatus?: string[] | null,
  MobileToken?: string,
...
export const exportAllDispatchData = async <T>(
