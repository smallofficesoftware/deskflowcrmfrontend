...
const TeamDispatchDataReportsView = ({
  ...
  const handleMultiPrint = async () => {
    ...
    try {
      const pdfmeOn = await isPdfmeEnabledForDispatch();
      if (pdfmeOn) {
        const choices = await fetchDispatchPdfmeTemplates();
        if (choices.length > 1) {
          setPrintTemplateChoices(choices);
          setPendingPrintCartId(cartIdOrIds);
          return;
        }
        await generateAndPrintDispatchPdf(cartIdOrIds);
        return;
      }
  ...
  const printWithTemplate = async (templateId: number) => {
    ...
    setIsMultiPrintLoading(true);
    try {
      await generateAndPrintDispatchPdf(cartIdOrIds, templateId);
    } finally {
      setIsMultiPrintLoading(false);
    }
  };
