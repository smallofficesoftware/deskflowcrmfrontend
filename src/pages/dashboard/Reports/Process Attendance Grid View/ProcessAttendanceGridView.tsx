...
interface IPropsProcessAttendanceGridView {
    onHide: () => void;
}

const ProcessAttendanceGridView = ({
    onHide
}: IPropsProcessAttendanceGridView) => {
    ...
    useEffect(() => {
        if (hasMore && !isFetchingMore && processAttendanceList.length >= offset + PAGE_SIZE) {
            loadNextPage();
        }
    }, [hasMore, isFetchingMore, processAttendanceList.length, offset]);
...
export default ProcessAttendanceGridView;