...
const CategoryPendingReport = ({
  ...
  const filters = getFilter("category_wise_pending_report");
  ...
  const handleApplyFilters = (data: any) => {
    ...
    setFilters("category_wise_pending_report", updatedFilters);
  ...
  useEffect(() => {
    if (!filters.startSearchDate || !filters.endSearchDate) {
      ...
      setFilters("category_wise_pending_report", {
        ...filters,
  ...
  const exportPdf = () => {
    ...
    const tableData = dataToExport.map((customer) => ({
      ...
      pending_purchase: customer.pending_purchase ?? "-",
    }));
    ...
    autoTable(doc, {
      ...
      didDrawPage: (data: any) => {
        doc.text("Category Wise Pending Report", data.settings.margin.left, 10);
      },
  ...
  const exportExcel = async () => {
    try {
      ...
      const excelRows = (
        ...
        "Sales Invoice": item.salesinvoice || "-",
        "Pending Sales": item.pending_sales || "-",
      ...
      excelRows.push({
        ...
        "Sales Order": calculateColumnTotals(excelRows, "Sales Order"),
        "Sales Invoice": calculateColumnTotals(excelRows, "Sales Invoice"),
        "Pending Sales": calculateColumnTotals(excelRows, "Pending Sales"),
  ...
  //     "Pending Purchase": calculateColumnTotals(exportData, "pending_purchase"),
  //   };
  ...
  const printTable = () => {
    ...
    const totals = {
      ...
      pending_sales: calculateColumnTotals(tableData, "pending_sales"),
      ...
      pending_purchase: calculateColumnTotals(tableData, "pending_purchase"),
    };

    const printContent = `
      <html>
        <head>
          <title>Category Wise Pending Report</title>
          <style>
        ...
        <body>
          <h1>Category Wise Pending Report</h1>
          <table>
            <thead>
              <tr>
                <th>Category Name</th>
            ...
            <tbody>
              ${tableData
                ...
                <tr>
                  ...
                  <td class="text-right">${customer.pending_purchase ?? "-"}</td>
                </tr>
              `,
                )
                .join("")}
      ...
      >
        ...
        >
          ...
          <Column
            ...
            bodyStyle={{ textAlign: "right" }}
            body={(rowData: ICategorySalesData) => rowData.pending_sales ?? "-"}
            footer={() => calculateColumnTotals(customers, "pending_sales")}
            footerStyle={{
              position: "sticky",
              bottom: 0,
              zIndex: 1,
          ...
          <Column
            ...
            footer={() => calculateColumnTotals(customers, "pending_purchase")}
            footerStyle={{
              position: "sticky",
              bottom: 0,
              zIndex: 1,
...
export default CategoryPendingReport;
