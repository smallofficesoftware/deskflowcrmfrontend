...
export const fetchCartReport = async (
    ...
    try {
        const response = await axiosInstance.post("getDailySalesInv", requestedData);
...
export const flattenCartDataForExport = (apiItems: any[]): IFlatCartItem[] => {
    return apiItems.map((item: any) => {
        ...
        const customFieldsArray =
            (custom.fields || []).map((f: any) => ({
                fieldName: f.reference_column_name,
                fieldLabel: f.title,
                value: custom.values?.[f.reference_column_name] ?? "",
            })) || [];
        ...
        return {
            ...
            statusDetails: cart.statusDetails,

            taxable_amt: cart.taxable_amt,
            gst_amt: cart.gst_amt,
            tcs_amt: cart.tcs_amt,
            round_off: cart.round_off,
            grand_total: cart.grand_total,

            is_approve: cart.is_approve,

            // ✅ CONTACT DETAILS
            to_customer_name: contact.to_customer_name,
            to_customer_company_name: contact.to_customer_company_name,
            to_customer_phone: contact.to_customer_phone,

            // ✅ PRODUCTS (IMPORTANT)
            items: item.product_items || [],

            // ✅ CUSTOM FIELDS (flattened for export)
            customForm: customFieldsArray,

            // keep original also (optional)
            original: item,
        };
    });
};

export const fetchSalesInvoiceReportForExport = async (
    selectedDates: any,
    selectedTeamMembers: string[] | null,
    selectedStageStatus?: string[] | null,
    MobileToken?: string,
    ...
    const response = await axiosInstance.post("getDailySalesInv", payload);

    const rawItems = response?.data?.data?.item || [];

    return flattenCartDataForExport(rawItems);
};

export const exportAllSalesInvoiceData = async <T>(
