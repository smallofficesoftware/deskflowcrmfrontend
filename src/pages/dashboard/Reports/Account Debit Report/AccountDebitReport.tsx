...
import {
  exportAccountDebitData,
  fetchAccountTransactions,
  IAccountTransaction,
} from "./AccountDebitReportController"; // Reuse same controller (assumes it supports debit flag)
...
interface IPropAccountOutstandingReports {
  ...
  globalSearch?: string;
  credit_debit_flag?: number; // Use this to pass debit flag (e.g., 2 or whatever your backend uses for debit)
...
const AccountDebitReport = ({
  ...
  const filters = getFilter("account_debit_report");
  ...
  const handleApplyFilters = (data: any) => {
    ...
    setFilters("account_debit_report", updatedFilters);
  ...
  useEffect(() => {
    if (!filters.startSearchDate || !filters.endSearchDate) {
      ...
      setFilters("account_debit_report", {
        ...filters,
  ...
  //         credit_debit_flag // Pass debit flag here
  //       );
  //       setLoading(false);
  //     } catch (err) {
  ...
  //   };
  //   fetchData();
  // }, [selectedDates, MobileToken, getID, MobileFlag, selectedTeamMembers, debouncedGlobalSearch, credit_debit_flag]);
  ...
  useEffect(() => {
    ...
    let result = transactions.filter(
      (txn) => txn.typeItem?.toLowerCase() === "debit",
    );
  ...
  // Only sum Debit amounts
  const totalDebitAmount = filteredTransactions.reduce(
    (acc, txn) => {
  ...
  const exportPdf = () => {
    ...
    const tableData = dataToExport.map((txn) => ({
      ID: txn.acc_series,
      // "Contact ID": txn.contact_masters_id,
      // "Login ID": txn.a_application_login_id,
      // "Company ID": txn.company_masters_id,
  ...
  //    const exportData = dataToExport.map((txn: IAccountTransaction) => ({
  //      ID: txn.id ?? "",
  //      Contact_name: txn.contact_name ?? "",
  //      // Login_ID: txn.a_application_login_id ?? "",
  //      Company_name: txn.contact_companyName ?? "",
  //      Type: txn.typeItem ?? "",
  //      Mode: txn.modeItem ?? "",
  //      Amount: txn.amountwithcurrency || txn.amount || "",
  //      Payment_Date: formatDate(txn.payment_date_time),
  //      Remark: txn.remark ?? "",
  //      // Approver_ID: txn.approve_by_a_application_login_id ?? "",
  //      Approved_By: txn.approved_name ?? "",
  //      Approve_Date: formatDate(txn.approve_date_time),
  //      // Ref_ID: txn.reference_id ?? "",
  //      // Ref_Table: txn.reference_table ?? "",
  //      Created_Date_Time: formatDate(txn.created_date_time),
  //      // Timestamp: formatDate(txn.s_timestemp),
  ...
  //    saveAsExcelFile(excelBuffer, "account_transactions");
  //  };
  ...
  const exportExcel = async () => {
    try {
      ...
      const allTransactions = await exportAccountDebitData<IAccountTransaction>(
        (offset, limit) =>
          fetchAccountTransactions(
            filters.selectedDateArray,
            MobileToken,
      ...
      const exportData = (
        ...
        "Contanct Phone": txn.contact_mobileNumber ?? "",
        "Payment Type": txn.typeItem ?? "",
        "Payment Mode": txn.modeItem ?? "",
        [`Amount (${currencyName})`]:
          txn.amountwithoutcurrency || txn.amount || "",
            ...
            <tbody>
              ${data
                ...
                <tr>
                  ...
                  <td>${txn.contact_companyName || txn.contact_name || "-"} - ${txn.contact_mobileNumber || ""}</td>
                  <td>${txn.typeItem}</td>
                  <td>${txn.modeItem}</td>
      ...
      <div className="report_card" style={{ height: "90vh" }}>
        ...
        >
          ...
          <Column
            ...
            style={{ width: "150px" }}
            body={() => (
              <span style={{ color: "red", fontWeight: "bold" }}>Debit</span>
            )}
          />
...
export default AccountDebitReport;
