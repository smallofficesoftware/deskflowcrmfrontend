...
// Return Sales Invoice (cart type 6) is pdfme-supported - same shape as
...
export const isPdfmeEnabledForReturnSalesInvoice = async (): Promise<boolean> => {
  if (!isPdfmeSupportedCartType(6)) return false;
  ...
  try {
    const { data } = await axiosInstance.post("get-feature-flag", {
      ...
      feature_key: "returnSalesInvoice_document_designer",
    });
...
export const fetchReturnSalesInvoiceReportForExport = async (
  selectedDates: any,
  selectedTeamMembers: string[] | null,
  selectedStageStatus?: string[] | null,
  MobileToken?: string,
...
export const exportAllReturnSalesInvoiceData = async <T>(
