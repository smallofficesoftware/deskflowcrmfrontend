...
import {
  ...
  openPrint,
} from "./ReturnSalesInvoiceController";
...
const TeamReturnSalesDataReportsView = ({
  selectedDates,
  selectedTeamMembers,
  selectedStageStatus,
  selectedSeries,
  ...
  const filters = getFilter("return_sales_invoice");
  ...
  const handleApplyFilters = (data: any) => {
    ...
    setFilters("return_sales_invoice", updatedFilters);
  ...
  useEffect(() => {
    if (!filters.startSearchDate || !filters.endSearchDate) {
      ...
      setFilters("return_sales_invoice", {
        ...filters,
  ...
  const canDelReturnSalesInv = useCheckUserPermission(
    PAGE_ID.RETURN_SALES_INVOICE,
  ...
  const onVirtualScroll = async (
    ...
    try {
      const data = await fetchCartReport(
        ...
        MobileToken,
        getID,
        offsetRef.current, // correct offset
        PAGE_SIZE,
        debouncedSearchText,
        filters.checkedOptionsSeries,
        setCurrencyName,
  ...
  const handelChangeShowModelQuotation = (item: IUserList) => {
    if (canEditQuotation) {
      ...
      setIsOrderShowFromContactType(6);
  ...
  const {
    ...
    reorderColumns,
    resetColumns,
  } = useColumnPreferences("return_sales_invoice_report", baseColumnDefs);
  ...
  const handleMultiPrint = async () => {
    ...
    try {
      const pdfmeOn = await isPdfmeEnabledForReturnSalesInvoice();
      if (pdfmeOn) {
        const choices = await fetchReturnSalesInvoicePdfmeTemplates();
        if (choices.length > 1) {
          setPrintTemplateChoices(choices);
          setPendingPrintCartId(cartIdOrIds);
          return;
        }
        await generateAndPrintReturnSalesInvoicePdf(cartIdOrIds);
        return;
      }
  ...
  const printWithTemplate = async (templateId: number) => {
    ...
    setIsMultiPrintLoading(true);
    try {
      await generateAndPrintReturnSalesInvoicePdf(cartIdOrIds, templateId);
    } finally {
      setIsMultiPrintLoading(false);
    }
  };
...
export default TeamReturnSalesDataReportsView;
