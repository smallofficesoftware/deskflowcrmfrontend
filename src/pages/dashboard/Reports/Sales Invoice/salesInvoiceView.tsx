...
import {
  ...
  fetchCartReport,
  fetchSalesInvoicePdfmeTemplates,
  fetchSalesInvoiceReportForExport,
  generateAndPrintSalesInvoicePdf,
  IFlatCartItem,
  isPdfmeEnabledForSalesInvoice,
} from "./salesInvoiceController";
...
const TeamSalesInvoiceDataReportsView = ({
  selectedDates,
  selectedTeamMembers,
  title,
  setRefreshReport1,
  ...
  const filters = getFilter("order_invoice");
  ...
  const handleApplyFilters = (data: any) => {
    ...
    setFilters("order_invoice", updatedFilters);
  ...
  const canDelSalesInvoice = useCheckUserPermission(
    PAGE_ID.INVOICE,
  ...
  type SalesInvoiceColumnDef = ColumnDef & {
  ...
  const baseColumnDefs: SalesInvoiceColumnDef[] = useMemo(() => {
    const defs: SalesInvoiceColumnDef[] = [];
    ...
    defs.push(
      {
        ...
        body: (rowData: any) => (
          <span
            ...
            onClick={() => {
              if (canEditSalesInvoice && !MobileFlag) {
                handelChangeShowModesalesInvoice(rowData);
              } else if (!canEditSalesInvoice) {
      ...
      },
      {
        ...
        width: "125px",
        body: (rowData: any) => <div>{rowData.gst_number || "-"}</div>,
      },
      {
        key: "username",
        label: "Created By",
  ...
  const {
    ...
    reorderColumns,
    resetColumns,
  } = useColumnPreferences("sales_invoice_report", baseColumnDefs);

  const getExportCellValue = (
    col: SalesInvoiceColumnDef,
    item: any,
  ): string => {
    switch (col.key) {
      case "items":
  ...
  const fetchAccountOutstandingForExport = async (
    ...
    return fetchSalesInvoiceReportForExport(
      filters.selectedDateArray,
      filters.checkedOptionsUser,
      filters.checkedOptionsStageStatus,
      MobileToken,
  ...
  const exportExcel = async () => {
    try {
      ...
      xlsx.utils.book_append_sheet(wb, ws, "Sales Invoice Report");
      ...
      saveAs(
        ...
        `salesinvoice_report_${Date.now()}.xlsx`,
      );
  ...
  const handleMultiPrint = async () => {
    ...
    try {
      const pdfmeOn = await isPdfmeEnabledForSalesInvoice();
      if (pdfmeOn) {
        const choices = await fetchSalesInvoicePdfmeTemplates();
        if (choices.length > 1) {
          setPrintTemplateChoices(choices);
          setPendingPrintCartId(cartIdOrIds);
          return;
        }
        await generateAndPrintSalesInvoicePdf(cartIdOrIds);
        return;
      }
  ...
  const printWithTemplate = async (templateId: number) => {
    ...
    setIsMultiPrintLoading(true);
    try {
      await generateAndPrintSalesInvoicePdf(cartIdOrIds, templateId);
    } finally {
      setIsMultiPrintLoading(false);
    }
  };
            ...
            >
              ...
              {visibleColumns.map((col) => (
                <Column
                  ...
                  headerClassName={col.key === "items" ? "center-header" : undefined}
                  headerStyle={{
...
export default TeamSalesInvoiceDataReportsView;
