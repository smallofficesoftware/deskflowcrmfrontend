...
import { IEmpAccountTransaction } from "../left-side/header/Setting/employee-account-transaction/EmpAccountTransactionController";
import { fetchprintSetting, IprintSetting } from "../order-pdf-view/OrderPdfController";
import { fetchApiAccountTransitions } from "./EmpAccountTransactionV1Controller";
...
const EmpAccountTransactionV1 = () => {
    ...
    const [accountTransactionList, setAccountTransactionList] = useState<IEmpAccountTransaction[]>([]);
    ...
    useEffect(() => {
        ...
        fetchApiAccountTransitions(
            ...
            qrCode,
            (cdata?: EmployeeData) => {
                if (cdata) setEmployeeData(cdata);
            },
            startDate,
            endDate,
            creditFilter,
    ...
    const employeeFields = (cd: EmployeeData) => {
        const rows: { label: string; value?: string }[] = [
            { label: "Name", value: cd.username },
            { label: "Mobile", value: cd.recovery_mobile },
            { label: "Email", value: cd.recovery_email },
        ];
                        ...
                        <div className="col-6 text-start">
                            ...
                            {employeeData && (
                                <div className="mt-2 small text-start">
                                    {employeeData.username && <div><strong>{employeeData.username}</strong></div>}
                                    {employeeData.recovery_mobile && <div>Mo. {employeeData.recovery_mobile}</div>}
                                    {employeeData.recovery_email && <div>{employeeData.recovery_email}</div>}
                                </div>
                            )}
                        </div>
                    </div>
...
export default EmpAccountTransactionV1;
