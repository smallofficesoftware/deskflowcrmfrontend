...
import CreateVisitTypeView from "./CreateVisitTypeView";
import {
  fetchVisitTypeApi,
  handleDeleteVisitType,
  IVisitTypeView
} from "./VisitTypeController";

interface IPropsVisitTypeView {
  isVisitTypeView: boolean;
  closeVisitTypeView: () => void;
}
...
const VisitTypeView = ({
  ...
  const [visitTypeLists, setVisitTypeList] = useState<IVisitTypeView[]>([]);
  // const [visitTypeInput, setVisitTypeInput] = useState("");
  // const [visitTypeHexColorInput, setVisitTypeHexColorInput] = useState("#999999");
  ...
  // const [editVisitTypeId, setEditVisitTypeId] = useState<number | undefined>(undefined);
  ...
  useEscapeKey(closeVisitTypeView);
  ...
  //   setVisitTypeError("");
  //   if (visitTypeInput) {
  //     if (isEditing && editVisitTypeId !== undefined) {
  //       updateVisitType(
  //         {
  ...
  const toggleDropdownVisitType = (visitTypeId: number | undefined) => {
    if (visitTypeId === undefined) return;

    setIsActionDropdownOpen(false);

    setOpenDropdownId((prevId) => {
      return prevId === visitTypeId ? null : visitTypeId;
    });
  };
  ...
  useEffect(() => {
    if (canView && isVisitTypeView) {
      fetchVisitTypeApi(setVisitTypeList, setLoading);
    }
  }, [isVisitTypeView, canView]);

  const handleEdit = (item: IVisitTypeView) => {
    // setOpenDropdownId(null);
    if (canEdit) {
      // setVisitTypeDropdown({});
      // setVisitTypeInput(item.visit_type);
      // setVisitTypeHexColorInput(item.color || "#999999");
      // setIsEditing(true);
      // setEditVisitTypeId(item.id);
      // setVisitTypeError("");
  ...
  const toggleSelection = (id: number) => {
    setSelectedIds((prev) => {
      const newSelected = prev.includes(id)
        ? prev.filter((i) => i !== id)
        : [...prev, id];
      const totalSelectable = visitTypeLists.filter((c) => c.id !== -1).length;
  ...
  const openDeleteSelected = () => {
    if (selectedIds.length === 0) {
      toast.error("No visit types selected");
      return;
    }
    if (canDelete) {
          ...
          <div className="header-Chat">
            ...
            <div className="text-end mb-2">
              ...
              >
                <button
                  className="icons"
                  onClick={openCreateVisitTypeView}
                  title="Create Visit Type"
                >
                  <svg
              ...
              >
                <button
                  className="icons"
                  onClick={handleRefreshVisitType}
                  title="Refresh"
                >
                      ...
                      <>
                        <div className="source-of-type-list-grid-block">
                          <div className="source-of-type-list-grid-main">
                            ...
                            <p
                              className={`${visitTypeLists.length > 0 ? "" : "text-center pt-5"}`}
                            >
                              {visitTypeLists.length > 0 ? "" : "No Data Found"}
                            </p>
                            {visitTypeLists &&
                              visitTypeLists.map((item, index) => (
                                    ...
                                    <>
                                      <button
                                        ...
                                        onClick={(e) => {
                                          e.stopPropagation();
                                          setIsActionDropdownOpen(false);
                                          toggleDropdownVisitType(item.id);
                                        }}                                      >
