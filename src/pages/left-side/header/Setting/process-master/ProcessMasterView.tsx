...
import CreateProcessMasterView from "./CreateProcessMasterView";
import {
    fetchProcessApi,
    handleDeleteProcess,
    IProcessView
} from "./ProcessMasterController";

interface IPropsprocessMasterView {
    isprocessMasterView: boolean;
    closeprocessMasterView: () => void;
}
const ProcessMaster = ({
    isprocessMasterView,
    closeprocessMasterView,
}: IPropsprocessMasterView) => {
    const [processLists, setProcessList] = useState<IProcessView[]>([]);
    // const [processMasterInput, setProcessMasterInput] = useState("");
    // const [processHexColorInput, setprocessHexColorInput] = useState("#eeeeee");
    const [loading, setLoading] = useState(false);
    const { darkMode, toggleTheme } = useTheme();
    const processRefDropdown = useRef<HTMLButtonElement>(null);
    const dropdownContactRef = useRef<Record<number, HTMLUListElement | null>>(
        {}
    );
    const [processDropdown, setprocessDropdown] = useState<any>(null);
    const [hasIdAvail, setHasIdAvail] = useState<number>();
    const [isDeleteConfirmation, setIsDeleteConfirmation] = useState(false);
    // const [isEditing, setIsEditing] = useState<boolean>(false);
    // const [editprocessId, setEditprocessId] = useState<number | undefined>(
    //     undefined
    // );
    ...
    const [editableProduct, setEditableProductl] = useState<IProcessView>({
        process_name: "",
        id: 0,
        color: "",
        created_date_time: "",
    ...
    useEscapeKey(closeprocessMasterView);
    ...
    //         if (isEditing && editprocessId !== undefined) {
    //             updateprocess(
    //                 {
    ...
    const toggleDropdownprocess = (processId: number | undefined) => {
        const checkedId = processLists.find((abv) => abv.id === processId);
        if (checkedId) {
            setHasIdAvail((prev) => (prev === checkedId.id ? undefined : checkedId.id));
        }
        setprocessDropdown(!processDropdown);
    };
    ...
    useEffect(() => {
        if (canView) {
            if (isprocessMasterView) {
                fetchProcessApi(setProcessList, setLoading);
            }
        }
    }, [isprocessMasterView, canView]);

    const handleEdit = (item: IProcessView) => {
        setprocessDropdown(null);

        if (canEdit) {
            // setProcessMasterInput(item.process_name);
            // setprocessHexColorInput(item.color || "#eeeeee");
            // setIsEditing(true);
            // setEditprocessId(item.id);
            // setprocessError("");
            setEditableProductl(item);
                    ...
                    <div className="header-Chat">
                        ...
                        <div className="ICON">
                            <div
                                ...
                                title="Back"
                                aria-label="New chat"
                                onClick={closeprocessMasterView}
                            >
                                            ...
                                            <>
                                                <div className="source-of-type-list-grid-block">
                                                    <div className="source-of-type-list-grid-main">
                                                        <p
                                                            className={`${processLists.length > 0
                                                                ? ""
                                                                : " text-center pt-5"
                                                                }`}
                                                        >
                                                            {processLists.length > 0 ? "" : "No Data Found"}
                                                        </p>
                                                                ...
                                                                >
                                                                    ...
                                                                    {item.id === -1 ? (
                                                                        ...
                                                                        <>
                                                                            ...
                                                                            <ul
                                                                                className={`source-of-types-options ${hasIdAvail === item.id &&
                                                                                    processDropdown
                                                                                    ? "isVisible"
                                                                                    : "isHidden"
                                                                                    } `}
