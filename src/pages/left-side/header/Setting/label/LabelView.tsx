...
interface IPropsLabelView {
  isLableView: boolean;
  closeLabelView: () => void;
}

const LabelView = ({ isLableView, closeLabelView }: IPropsLabelView) => {
  ...
  const labelRefDropdown = useRef<HTMLButtonElement>(null);
  const dropdownContactRef = useRef<Record<number, HTMLUListElement | null>>(
    {}
  );
  // const [labelDropdown, setLabelDropdown] = useState<any>({});
  const [openDropdownId, setOpenDropdownId] = useState<number | null>(null);
  const [deleteItemIds, setDeleteItemIds] = useState<number[]>([]);
  const [isDeleteConfirmation, setIsDeleteConfirmation] = useState(false);
  // const [isEditing, setIsEditing] = useState<boolean>(false);
  // const [editLabelId, setEditLabelId] = useState<number | undefined>(undefined);
  ...
  //   setlabelError("");
  //   if (labelInput) {
  //     if (isEditing && editLabelId !== undefined) {
  //       updateLabel(
  //         {
  ...
  const toggleDropdownLabel = (labelId: number | undefined) => {
    ...
    setOpenDropdownId((prevId) => {
      return prevId === labelId ? null : labelId;
    });
  };
  ...
  const handleEdit = (item: ILabelView) => {
    setOpenDropdownId(null);
    if (canEdit) {
      // setLabelInputInput(item.lable_name);
      // setLabelHexColorInput(item.color || "#999999");
      // setIsEditing(true);
      // setEditLabelId(item.id);
      // setlabelError("");
      // setIsLabelInputReadOnly(item.id < 0);
                      ...
                      <>
                        <div className="source-of-type-list-grid-block">
                          <div className="source-of-type-list-grid-main">
                            ...
                            <p
                              className={`${labelLists.length > 0 ? "" : "text-center pt-5"
                                }`}
                            >
                              {labelLists.length > 0 ? "" : "No Data Found"}
                            </p>
                            {labelLists &&
