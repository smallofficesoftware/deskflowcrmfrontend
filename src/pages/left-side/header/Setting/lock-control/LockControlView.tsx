...
import AddLockControlView from "./AddLockControlView";
import { deleteLockControl, fetchLockControlApi, ILockControlView } from "./LockControlController";

export interface IPropsHolidayView {
  isLockControlView: boolean;
  closeLockControlView: () => void;
}

const LockControlView = ({
  isLockControlView,
  closeLockControlView,
}: IPropsHolidayView) => {
  const [lockControlList, setLockControlList] = useState<ILockControlView[]>([]);
  ...
  const [editableLockControl, setEditableLockControl] = useState<
    ILockControlView | undefined
  >();
  ...
  useEffect(() => {
    const fetchLockControls = async () => {
      if (isLockControlView && canView) {
        setOffset(0);
        setHasMore(true);
        setLockControlList([]);
        setLoading(true);
  ...
  const handleDeleteLockControl = async () => {
    ...
    await deleteLockControl(deleteLockControlIds, setIsDeleteConfirmation, setLoading);
          ...
          <div className="header-Chat">
            ...
            <div className="text-end mb-2">
              ...
              >
                <button
                  className="icons"
                  onClick={handleRefreshLockControl}
                  title="Refresh"
                >
                  ...
                  <div>
                    {loading ? (
                      ...
                      <>
                        ...
                        >
                          {lockControlList.length === 0 ? (
                            <p className="text-center pt-5">No Data Found</p>
                          ) : (
                            lockControlList.map((item) => (
                              <div
                                key={item.id}
                                      ...
                                      >
                                        ...
                                        <li
                                          ...
                                          onClick={() => {
                                            setOpenDropdownId(null);
                                            if (canDelete) {
                                              setDeleteLockControlIds([item.id]);
