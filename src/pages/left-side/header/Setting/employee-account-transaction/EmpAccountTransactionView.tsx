...
import { DEFAULT_MESSAGE_ERROR_PERMISSION, DEFAULT_STATUS_CODE_SUCCESS, ITEMS_PER_PAGE, MESSAGE_UNKNOWN_ERROR_OCCURRED } from "../../../../../helpers/AppConstants";
...
interface IPropsListEmpAccountTransaction {
...
const ListEmpAccountTransactionView = ({
    isListAccountTransaction,
    closeListAccountTransaction,
    teamId: teamId,
    setRefreshAccount,
    companyTeamInfo
}: IPropsListEmpAccountTransaction) => {
    ...
    const [isSingleSendingToWhatsApp, setIsSingleSendingToWhatsApp] = useState(false);
    const [currentPage, setCurrentPage] = useState(0);
    const [searchTerm, setSearchTerm] = useState<string>("");
    const { darkMode, toggleTheme } = useTheme();
    const [noDataFound, setNoDataFound] = useState(false);
    const [accountTransactionList, setAccountTransactionList] = useState<
        IEmpAccountTransaction[]
    >([]);
        ...
        try {
            ...
            if (data.code === 200) {
                if (data.ack === DEFAULT_STATUS_CODE_SUCCESS) {
                    ...
                    await sendToWhatsApp({ numbers: [data.data.mobile_number], sessionName: data.data.sessionName, mediaUrls: fileUrl, type: "document", mimetype: "application/pdf" });
                }
            } else {
    ...
    const handleApproveAccountTransaction = async () => {
        try {
            ...
            const requestData = {
                approveId: selectedIds.length > 0
                    ? selectedIds
                    : [approveId],
    ...
    const handleConfirmFilter = async (filterPayload: IFilterPayload) => {
        const { startSearchDate, endSearchDate, initialCheckedShowCreditData, initialCheckedShowDebitData } = filterPayload;
        setFilterParams({
            startSearchDate: startSearchDate instanceof DateObject ? startSearchDate.format("YYYY-MM-DD") : startSearchDate,
            endSearchDate: endSearchDate instanceof DateObject ? endSearchDate.format("YYYY-MM-DD") : endSearchDate,
    ...
    const handelChangeAccountTractionEdit = (itemId: IEmpAccountTransaction) => {
        if (canEdit) {
    ...
    useEffect(() => {
        if (!canView) return;
        if (isListAccountTransaction && teamId) {
            fetchApiAccountTransitions(
                0,
                "",
                setAccountTransactionList,
    ...
    return (
        <>
                    ...
                    <div className="leftSide " id="search-message">
                        <div className="header-Chat">
                            ...
                            <div className="newText " style={{ width: "90%" }}>
                                <h2>Account History of {companyTeamInfo?.username}</h2>
                            </div>
                  ...
                  <span className="text-white" title="Download">
                    ...
                    >
                      <path d="M480-320 280-520l56-58 104 104v-326h80v326l104-104 56 58-200 200ZM240-160q-33 0-56.5-23.5T160-240v-120h80v120h480v-120h80v120q0 33-23.5 56.5T720-160H240Z" />
                                ...
                                <div className="text-left" style={{ position: "absolute", right: "2%", top: "-60%", zIndex: "1000", }}>
                                    <ul
                                    ...
                                    >
                                        <li
                                            ...
                                            role="button"
                                            onClick={() => { !isAllPDFDownloadLoading && downloadAllTransactionOfEmployeePDF() }}
                        ...
                        >
                            <div className="row p-2">
                                {loading && isListAccountTransaction && teamId ? (
                                                                            ...
                                                                            {canDelete && (
                                                                                <li
                                                                                    ...
                                                                                    onClick={(e) => {
                                                                                        e.stopPropagation();
                                                                                        setDebitDropdownOpen(null);
                                                                                        handelChangeAccountTractionDelete(item.id);
                                                                                    }}
                                                                                >
                                                                                    Delete
                                                                                </li>
                                                                        ...
                                                                        <>
                                                                            ...
                                                                            <li
                                                                                ...
                                                                                onClick={(e) => {
                                                                                    e.stopPropagation();
                                                                                    setDebitDropdownOpen(null);
                                                                                    if (!isPDFDownloadLoading) downloadPDF(item.id);
                                                                                }}
                                                                            >
                                                                                {isPDFDownloadLoading ? "Downloading..." : "Download PDF"}
                                                                            </li>
                                                                        </>
                                                                    )}
                                                                </ul>
                                                            </div>
                                                            <button key={index} className={`block chat-list ${activeIndex === index ? "active" : ""}`} style={{ padding: "6" }} onClick={(e) => {
                                                                ...
                                                                <div className="h-text">
                                                                    <div className="text-start">
                                                                        <h4 className="account-transaction-front-id ">
                                                                            <span className={`${checkboxesVisible ? "ms-4" : ""}`}>{"#" + item.id}</span>
                                                                            <br />
                                                                        </h4>
                                                                    </div>
                                                                    <div className="text-start">
                                                                        ...
                                                                        <div className="text-end">
                                                                            ...
                                                                            <p className="contact-text">
                                                                                {item.a_application_login_name && (
                                                                                    <>
                                                                                        Created By: {item.a_application_login_name}
                                                                                        <br />
                                                                                    </>
                                                                                )}
                                                                                {item.approve_by_a_application_login_name && (
                                                                                    <>Approved By: {item.approve_by_a_application_login_name}</>
                                                                                )}
                                                                            </p>
                                                        ...
                                                        <>
                                                            ...
                                                            >
                                                                ...
                                                                <div className="h-text">
                                                                    <div>
                                                                        <ul
                                                                            style={{ width: "200px", height: "160px", textAlign: "left" }}
                                                                    ...
                                                                    <div className="text-end">
                                                                        ...
                                                                        <h4 className="account-transaction-front-id ">
                                                                            <span className={`${checkboxesVisible ? "ms-3" : ""}`}>{"#" + item.id}</span>
                                                                            <br />
                                                                        </h4>
                                                                    </div>
                                                                    <div className="text-end">
            ...
            <CreateEmpAccountTransactionView
...
export default ListEmpAccountTransactionView;